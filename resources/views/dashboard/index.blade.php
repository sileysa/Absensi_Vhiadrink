...
@section('content')
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-emerald-900">Pilih Stand Absensi</h1>
    ...
    @if ($selectedStand)
        <div class="mb-8 rounded-2xl border border-emerald-200 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm font-medium uppercase tracking-wide text-emerald-600">Stand Terpilih</p>
                ...
                <div class="flex flex-wrap gap-3">
                    @auth
                        <a href="{{ route('attendance.index') }}" class="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white hover:bg-emerald-700">
                            Lanjut Absensi
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white hover:bg-emerald-700">
                            Login untuk Absensi
                        </a>
                    @endauth
                        ...
                        @foreach ($regionStands as $stand)
                            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-emerald-300 hover:shadow-md">
                                ...
                                <form method="POST" action="{{ route('stand.select') }}" class="mt-5">
                                    ...
                                    <button
                                        type="submit"
                                        class="w-full rounded-xl px-4 py-2.5 text-sm font-semibold {{ $selectedStand?->id === $stand->id ? 'bg-emerald-100 text-emerald-800' : 'bg-emerald-600 text-white hover:bg-emerald-700' }}"
                                    >
